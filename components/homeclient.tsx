...
import { useAuth } from "@/hooks/useAuth"; // your auth hook (upgraded version)
...
export default function HomeClient() {
  const { isAuthenticated, user, login, signup, loading: authLoading } = useAuth();
  ...
  const checkout = async (planId: string) => {
    ...
    try {
      await trackEvent("checkout_click", { planId, userId: user?.id });
      ...
      {showAuthModal && (
        <div style={styles.modalOverlay} onClick={() => setShowAuthModal(false)}>
          <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
            <h3 style={styles.modalTitle}>{authMode === "login" ? "Log in" : "Sign up"}</h3>
